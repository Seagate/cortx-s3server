...
class S3GetBucketVersioningActionTest : public testing::Test {
 protected:  // You should make the members protected s.t. they can be
  ...
  S3GetBucketVersioningActionTest() {
    ...
    action_under_test_ptr = std::make_shared<S3GetBucketVersioningAction>(
        request_mock, bucket_meta_factory);
    mock_request_id.assign("mock_request_id");
    mock_bucket_version_str.assign("MockBucketVersion");

    call_count_one = 0;
  }

  std::shared_ptr<MockS3RequestObject> request_mock;
  std::shared_ptr<S3GetBucketVersioningAction> action_under_test_ptr;
...
TEST_F(S3GetBucketVersioningActionTest, SendResponseToClientNoSuchBucket) {
...
TEST_F(S3GetBucketVersioningActionTest, SendResponseToClientInternalError) {
...
TEST_F(S3GetBucketVersioningActionTest, SendResponseToClientSuccess) {
  action_under_test_ptr->bucket_metadata =
      bucket_meta_factory->mock_bucket_metadata;
