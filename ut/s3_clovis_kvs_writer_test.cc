...
int s3_test_alloc_op(struct m0_clovis_entity *entity, struct m0_clovis_op **op) {
...
void s3_test_free_op(struct m0_clovis_op* op) {
  free(op);
}
...
TEST_F(S3ClovisKvsWritterTest, CreateIndexSuccessCallback) {
  ...
  EXPECT_CALL(*ptr_mock_s3clovis, clovis_entity_create(_, _)).WillOnce(Invoke(s3_test_alloc_op));
