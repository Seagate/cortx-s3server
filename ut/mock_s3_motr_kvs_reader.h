...
class MockS3MotrKVSReader : public S3MotrKVSReader {
 public:
  ...
  MOCK_CONST_METHOD0(get_state, S3MotrKVSReaderOpState());
  MOCK_CONST_METHOD0(get_value, const std::string &());
  MOCK_CONST_METHOD0(
      get_key_values,
      const std::map<std::string, std::pair<int, std::string>> &());
  MOCK_METHOD3(lookup_index, void(const struct s3_motr_idx_layout &,
                                  std::function<void(void)>,
                                  std::function<void(void)> on_failed));
  MOCK_METHOD4(get_keyval, void(const struct s3_motr_idx_layout &,
