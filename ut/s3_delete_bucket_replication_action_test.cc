...
class S3DeleteBucketReplicationActionTest : public testing::Test {
 protected:  // You should make the members protected s.t. they can be
             // accessed from sub-classes.
  S3DeleteBucketReplicationActionTest() {
    ...
    action_under_test_ptr = std::make_shared<S3DeleteBucketReplicationAction>(
...
TEST_F(S3DeleteBucketReplicationActionTest, Constructor) {
  EXPECT_NE(0, action_under_test_ptr->number_of_tasks());
}

TEST_F(S3DeleteBucketReplicationActionTest, DeleteBucketReplicationSuccessful) {
  ...
  EXPECT_CALL(*(bucket_meta_factory->mock_bucket_metadata),
              delete_bucket_replication_config()).Times(1);
  EXPECT_CALL(*(bucket_meta_factory->mock_bucket_metadata), update(_, _))
      .Times(1);

  action_under_test_ptr->delete_bucket_replication();
}

TEST_F(S3DeleteBucketReplicationActionTest,
...
TEST_F(S3DeleteBucketReplicationActionTest,
       DeleteBucketReplicationFailed_Metadatafailed) {
  call_count_one = 0;
  action_under_test_ptr->bucket_metadata =
      action_under_test_ptr->bucket_metadata_factory
          ->create_bucket_metadata_obj(request_mock);
  ...
  ACTION_TASK_ADD_OBJPTR(action_under_test_ptr,
                         ...
                         this);
  action_under_test_ptr->delete_bucket_replication_failed();
  EXPECT_EQ(1, call_count_one);
}

TEST_F(S3DeleteBucketReplicationActionTest,
       DeleteBucketReplicationFailed_Metadatafailedtolaunch) {
  call_count_one = 0;
  action_under_test_ptr->bucket_metadata =
      action_under_test_ptr->bucket_metadata_factory
          ->create_bucket_metadata_obj(request_mock);
...
TEST_F(S3DeleteBucketReplicationActionTest, SendResponseToClientNoSuchBucket) {
...
TEST_F(S3DeleteBucketReplicationActionTest, SendResponseToClientSuccess) {
  action_under_test_ptr->bucket_metadata =
      action_under_test_ptr->bucket_metadata_factory
          ->create_bucket_metadata_obj(request_mock);
...
TEST_F(S3DeleteBucketReplicationActionTest, SendResponseToClientInternalError) {
