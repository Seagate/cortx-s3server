...
const char triple_ascii_encoded[] =
...
TEST(Base64, TripleAscii) {
  ...
  EXPECT_EQ(encoded_length, sizeof(triple_ascii_encoded) - 1);
  EXPECT_FALSE(memcmp(s_encoded.c_str(), triple_ascii_encoded, encoded_length));
