...
class S3GetBucketReplicationActionTest : public testing::Test {
 protected:  // You should make the members protected s.t. they can be
             // accessed from sub-classes.
  S3GetBucketReplicationActionTest() {
    ...
    action_under_test_ptr = std::make_shared<S3GetBucketReplicationAction>(
        request_mock, bucket_meta_factory);
  }

  std::shared_ptr<MockS3RequestObject> request_mock;
  std::shared_ptr<S3GetBucketReplicationAction> action_under_test_ptr;
  std::shared_ptr<MockS3BucketMetadataFactory> bucket_meta_factory;
  std::string bucket_name;
};

TEST_F(S3GetBucketReplicationActionTest, Constructor) {
  EXPECT_NE(0, action_under_test_ptr->number_of_tasks());
}

TEST_F(S3GetBucketReplicationActionTest,
       SendResponseToClientNoSuchReplicationError) {
  ...
  action_under_test_ptr->set_s3_error("NoSuchReplicationConfiguration");
...
TEST_F(S3GetBucketReplicationActionTest, SendResponseToClientNoSuchBucket) {
  action_under_test_ptr->bucket_metadata =
      action_under_test_ptr->bucket_metadata_factory
          ->create_bucket_metadata_obj(request_mock);
...
TEST_F(S3GetBucketReplicationActionTest, SendResponseToClientSuccess) {
  action_under_test_ptr->bucket_metadata =
      action_under_test_ptr->bucket_metadata_factory
          ->create_bucket_metadata_obj(request_mock);
...
TEST_F(S3GetBucketReplicationActionTest, SendResponseToClientInternalError) {
  action_under_test_ptr->bucket_metadata =
      action_under_test_ptr->bucket_metadata_factory
          ->create_bucket_metadata_obj(request_mock);
