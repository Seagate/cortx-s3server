...
TEST_F(S3PutBucketActionTest, CreateBucketAlreadyExist) {
  ...
  action_under_test_ptr->bucket_metadata->add_system_attribute("Owner-Account-id", "54321");
...
TEST_F(S3PutBucketActionTest, CreateBucketAlreadyExistForAccount) {
  ...
  EXPECT_CALL(*request_mock, send_response(409, _)).Times(AtLeast(1));
  request_mock->set_account_id("12345");
  action_under_test_ptr->bucket_metadata->add_system_attribute("Owner-Account-id", "12345");
  action_under_test_ptr->create_bucket();
