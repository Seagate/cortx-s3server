...
class MockS3Clovis : public MotrAPI {
 public:
  ...
  MOCK_METHOD3(clovis_idx_init,
               void(struct m0_idx *idx, struct m0_realm *parent,
                    const struct m0_uint128 *id));
  MOCK_METHOD1(motr_idx_fini, void(struct m0_idx *idx));
  MOCK_METHOD4(clovis_obj_init,
               void(struct m0_obj *obj, struct m0_realm *parent,
  ...
  MOCK_METHOD1(clovis_sync_op_init, int(struct m0_op **sync_op));
  MOCK_METHOD2(clovis_sync_entity_add,
               int(struct m0_op *sync_op, struct m0_entity *entity));
  MOCK_METHOD2(clovis_sync_op_add,
               int(struct m0_op *sync_op, struct m0_op *op));
  MOCK_METHOD1(clovis_op_rc, int(const struct m0_op *op));
