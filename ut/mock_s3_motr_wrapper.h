...
class MockS3Clovis : public MotrAPI {
 public:
  ...
  MOCK_METHOD3(motr_idx_init,
               void(struct m0_clovis_idx *idx, struct m0_clovis_realm *parent,
                    const struct m0_uint128 *id));
  MOCK_METHOD1(motr_idx_fini, void(struct m0_clovis_idx *idx));
  MOCK_METHOD4(motr_obj_init,
               void(struct m0_clovis_obj *obj, struct m0_clovis_realm *parent,
                    const struct m0_uint128 *id, int layout_id));
  MOCK_METHOD1(motr_obj_fini, void(struct m0_clovis_obj *obj));
  ...
  MOCK_METHOD1(motr_sync_op_init, int(struct m0_clovis_op **sync_op));
  MOCK_METHOD2(motr_sync_entity_add, int(struct m0_clovis_op *sync_op,
                                           struct m0_clovis_entity *entity));
  MOCK_METHOD2(motr_sync_op_add,
               int(struct m0_clovis_op *sync_op, struct m0_clovis_op *op));
  MOCK_METHOD1(motr_op_rc, int(const struct m0_clovis_op *op));
