...
def test_get_no_oid():
    ...
    response = CORTXS3ObjectApi(config, CONNECTION_TYPE_PRODUCER).get(None)
...
def test_get_success():
    ...
    config = CORTXS3Config()
    response = CORTXS3ObjectApi(config, CONNECTION_TYPE_PRODUCER, connection=httpconnection).get("test_object1")
    if (response is not None):
...
def test_get_failure():
    ...
    config = CORTXS3Config()
    response = CORTXS3ObjectApi(config, CONNECTION_TYPE_PRODUCER, connection=httpconnection).get("test_index2")
    if (response is not None):
        assert response[0] is False
...
def test_put_success():
    ...
    config = CORTXS3Config()
    response = CORTXS3ObjectApi(config, CONNECTION_TYPE_PRODUCER, connection=httpconnection).put("test_index1", "test_value1")
    if (response is not None):
        assert response[0] is True
...
def test_put_failure():
    ...
    config = CORTXS3Config()
    response = CORTXS3ObjectApi(config, CONNECTION_TYPE_PRODUCER, connection=httpconnection).put("test_index2", "test_value2")
    if (response is not None):
        assert response[0] is False
...
def test_put_no_index():
    ...
    config = CORTXS3Config()
    response = CORTXS3ObjectApi(config, CONNECTION_TYPE_PRODUCER).put(None, "test_value2")
    if (response is not None):
        assert response[0] is False
...
def test_delete_success():
    ...
    config = CORTXS3Config()
    response = CORTXS3ObjectApi(config, CONNECTION_TYPE_PRODUCER, connection=httpconnection).delete("test_oid1", "test_layout_id1", "test_pvid_str")
    if (response is not None):
        assert response[0] is True
...
def test_delete_failure():
    ...
    config = CORTXS3Config()
    response = CORTXS3ObjectApi(config, CONNECTION_TYPE_PRODUCER, connection=httpconnection).delete("test_oid2", "test_layout_id2", "test_pvid_str")
    if (response is not None):
        assert response[0] is False
...
def test_delete_no_oid():
    ...
    config = CORTXS3Config()
    response = CORTXS3ObjectApi(config, CONNECTION_TYPE_PRODUCER).delete(None, "test_layot_id2", "test_pvid_str")
    if (response is not None):
        assert response[0] is False
        assert response[1] is None
...
def test_delete_no_layout_id():
    ...
    config = CORTXS3Config()
    response = CORTXS3ObjectApi(config, CONNECTION_TYPE_PRODUCER).delete("test_oid1", None, "test_pvid_str")
    if (response is not None):
        assert response[0] is False
        assert response[1] is None
...
def test_delete_no_pvid():
    ...
    config = CORTXS3Config()
    response = CORTXS3ObjectApi(config, CONNECTION_TYPE_PRODUCER).delete("test_oid1", "test_layout_id2", None)
    if (response is not None):
        assert response[0] is False
        assert response[1] is None
...
def test_head_success():
    ...
    config = CORTXS3Config()
    response = CORTXS3ObjectApi(config, CONNECTION_TYPE_PRODUCER, connection=httpconnection).head("test_oid1", "test_layout_id1")
    if (response is not None):
        assert response[0] is True
...
def test_head_failure():
    ...
    config = CORTXS3Config()
    response = CORTXS3ObjectApi(config, CONNECTION_TYPE_PRODUCER, connection=httpconnection).head("test_oid2", "test_layout_id2")
    if (response is not None):
        assert response[0] is False
...
def test_head_no_oid():
    ...
    config = CORTXS3Config()
    response = CORTXS3ObjectApi(config, CONNECTION_TYPE_PRODUCER).head(None, "test_layot_id2")
    if (response is not None):
        assert response[0] is False
        assert response[1] is None
...
def test_head_no_layout_id():
    ...
    config = CORTXS3Config()
    response = CORTXS3ObjectApi(config, CONNECTION_TYPE_PRODUCER).head("test_oid1", None)
    if (response is not None):
        assert response[0] is False
        assert response[1] is None
