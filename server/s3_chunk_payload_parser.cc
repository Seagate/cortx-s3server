...
S3ChunkDetail::S3ChunkDetail() :
  chunk_size(0), ready(false), chunk_number(0) {
}
...
void S3ChunkDetail::debug_dump() {
  s3_log(S3_LOG_DEBUG, "Chunk Details start: chunk_number = [%d], size = [%zu],\n\
         signature = [%s]\nHash = [%s]\nChunk Details end.\n",
         chunk_number, chunk_size, signature.c_str(), payload_hash.c_str());
}

bool S3ChunkDetail::is_ready() {
  return ready;
}

void S3ChunkDetail::add_size(size_t size) {
  chunk_size = size;
}

void S3ChunkDetail::add_signature(const std::string& sign) {
  signature = sign;
}
...
bool S3ChunkDetail::update_hash(evbuf_t *buf) {
  if (buf == NULL) {
    ...
    bool status = hash_ctx.Update(emptee_string.c_str(), emptee_string.length());
    return status;
  }
  ...
  /* do the actual peek */
  struct evbuffer_iovec *vec_in = (struct evbuffer_iovec *)malloc(num_of_extents * sizeof(struct evbuffer_iovec));
  ...
  for (size_t i = 0; i < num_of_extents; i++) {
    status = hash_ctx.Update((const char*)vec_in[i].iov_base, vec_in[i].iov_len);
    if (!status) {
      break;
    }
  }
...
size_t S3ChunkDetail::get_size() {
  return chunk_size;
}

const std::string& S3ChunkDetail::get_signature() {
  return signature;
}

std::string S3ChunkDetail::get_payload_hash() {
  return payload_hash;
}


S3ChunkPayloadParser::S3ChunkPayloadParser() : parser_state(ChunkParserState::c_start),
...
std::vector<evbuf_t *> S3ChunkPayloadParser::run(evbuf_t *buf) {
  ...
  while (true) {
    ...
    for (size_t i = 0; i < num_of_extents; i++) {
      // std::string chunk_debug((const char*)vec_in[i].iov_base, vec_in[i].iov_len);
        ...
        switch (parser_state) {
          ...
          case ChunkParserState::c_chunk_size: {
            ...
            if (chptr[j] == ';') {
              ...
              chunk_sig_key_char_state = chunk_sig_key_q_const;
              chunk_data_size_to_read = strtol (current_chunk_size.c_str(), NULL, 16);
          ...
          case ChunkParserState::c_chunk_signature_value: {
            // s3_log(S3_LOG_DEBUG, "ChunkParserState::c_chunk_signature_value.")
            const char* chptr = (const char*)vec_in[i].iov_base;
          ...
          case ChunkParserState::c_chunk_data: {
            ...
            // 1. current payload has "all" and "only" data related to current chunk, optionally crlf
            // 2. current payload has "partial" and "only" data related to current chunk.
            // 3. current payload had "all" data of current chunk and some part of next chunk
            if (chunk_data_size_to_read > 0) {
              ...
              if (chunk_data_size_to_read == remaining_to_parse) {
                ...
                // (>)  we have partial data, but its all part of data OR
                  ...
                  free(vec_in);
                  return return_val;
              } else if (chunk_data_size_to_read + 2 /*CRLF*/ == remaining_to_parse ) {
                // we have all data + crlf
                evbuf_t *current_buf = evbuffer_new();
                evbuffer_remove_buffer(buf, current_buf, chunk_data_size_to_read); //copy data
                ...
                free(vec_in);
                return return_val;
              } else if (chunk_data_size_to_read + 1 /*CR*/ == remaining_to_parse ) {
                // we have all data + cr
                evbuf_t *current_buf = evbuffer_new();
                ...
                evbuffer_remove_buffer(buf, current_buf, chunk_data_size_to_read);
                s3_log(S3_LOG_DEBUG, "After move evbuffer_get_length(buf) = %zu\n", evbuffer_get_length(buf));
                s3_log(S3_LOG_DEBUG, "After move evbuffer_get_length(current_buf) = %zu\n", evbuffer_get_length(current_buf));
