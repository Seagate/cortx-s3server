...
bool S3Option::load_section(std::string section_name,
  ...
  if (force_override_from_config) {
    if (section_name == "S3_SERVER_CONFIG") {
      ...
      S3_OPTION_ASSERT_AND_RET(s3_option_node,
                               "S3_AUDIT_LOGGER_KAFKA_BRIDGE_PATH");
      audit_logger_kafka_bridge_path =
          s3_option_node["S3_AUDIT_LOGGER_KAFKA_BRIDGE_PATH"].as<std::string>();
...
void S3Option::dump_options() {
  ...
  s3_log(S3_LOG_INFO, "", "S3_AUDIT_LOGGER_KAFKA_BRIDGE_PATH = %s\n",
         audit_logger_kafka_bridge_path.c_str());
...
std::string S3Option::get_audit_logger_kafka_bridge_path() {
  return audit_logger_kafka_bridge_path;
}
