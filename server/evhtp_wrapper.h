...
#ifndef __MERO_FE_S3_SERVER_EVHTP_WRAPPER_H__
#define __MERO_FE_S3_SERVER_EVHTP_WRAPPER_H__
...
class EvhtpInterface {
  public:
    ...
    virtual int http_kvs_for_each(evhtp_kvs_t * kvs, evhtp_kvs_iterator cb, void * arg) = 0;
    virtual const char * http_header_find(evhtp_headers_t * headers, const char * key) = 0;

    virtual void http_headers_add_header(evhtp_headers_t * headers, evhtp_header_t * header) = 0;
    ...
    virtual void http_send_reply_start(evhtp_request_t * request, evhtp_res code) = 0;
...
public:
  ...
  evhtp_header_t * http_header_new(const char * key, const char * val,
