...
bool S3URI::is_service_api() {
  return is_service_api;
}

bool S3URI::is_bucket_api(); {
  return is_bucket_api;
}

bool S3URI::is_object_api(); {
  return is_object_api;
}
...
S3OperationCode S3URI::operation_code() {
...
S3VirtualHostStyleURI::S3VirtualHostStyleURI(std::shared_ptr<S3RequestObject> req) : S3URI(uri) {
...
void S3VirtualHostStyleURI::setup_bucket_name() {
  if (host_header.find(S3Config::instance()->default_endpoint()) != std::string::npos) {
    bucket_name = host_header.substr(0, (host_header.length() - default_endpoint.length() - 1));
  }
