...
S3ClovisWriter::S3ClovisWriter(std::shared_ptr<S3RequestObject> req,
    ...
    : request(req), oid(object_id), state(S3ClovisWriterOpState::start) {
...
void S3ClovisWriter::write_content(std::function<void(void)> on_success,
                                   std::function<void(void)> on_failed,
  ...
  if (buffer.is_freezed() && buffer.length() < clovis_write_payload_size) {
    ...
    estimated_write_length = (buffer.length() / clovis_write_payload_size) *
  ...
  std::deque<std::tuple<void *, size_t> > data_items =
      buffer.get_buffers_ref(estimated_write_length);

  size_t clovis_block_count =
      (estimated_write_length + (clovis_block_size - 1)) / clovis_block_size;
  ...
  copy_timer.stop();
  LOG_PERF(("copy_to_clovis_buf_" + std::to_string(total_written - last_w_cnt) +
            "_bytes_ns")
               .c_str(),
  ...
  while (!data_items.empty()) {
    ...
    while (1) /* consume all data in extent */
    {
      if (pending_from_current_extent == data_to_consume) {
        /* consume all */
        // s3_log(S3_LOG_DEBUG, "Writing @ %d bytes from extent = [%d] in block
        // [%d] at index [%d]\n", pending_from_current_extent, i,
        // current_block_idx, idx_within_block);
        // s3_log(S3_LOG_DEBUG, "memcpy(%p, %p, %d)\n",
        // data.ov_buf[current_block_idx] + idx_within_block, data_ptr +
        // idx_within_extent, pending_from_current_extent);
        source = (char *)data_ptr + idx_within_extent;
        destination =
            (char *)rw_ctx->data->ov_buf[current_block_idx] + idx_within_block;
      ...
      {
        // s3_log(S3_LOG_DEBUG, "Writing @ %d bytes from extent = [%d] in block
        // [%d] at index [%d]\n", data_to_consume, i, current_block_idx,
        // idx_within_block);
        // s3_log(S3_LOG_DEBUG, "memcpy(%p, %p, %d)\n",
        // data.ov_buf[current_block_idx] + idx_within_block, data_ptr +
        // idx_within_extent, data_to_consume);
        source = (char *)data_ptr + idx_within_extent;
        destination =
            (char *)rw_ctx->data->ov_buf[current_block_idx] + idx_within_block;
        ...
        pending_from_current_extent =
            pending_from_current_extent - data_to_consume;
        data_to_consume = clovis_block_size;
        current_block_idx++;
      }
    }
