...
std::string S3AccountUserIdxMetadata::get_account_id() {
  return account_id;
}

std::string S3AccountUserIdxMetadata::get_user_name() {
  return user_name;
}

std::string S3AccountUserIdxMetadata::get_user_id() {
  return user_id;
}
...
void S3AccountUserIdxMetadata::load(std::function<void(void)> on_success, std::function<void(void)> on_failed) {
...
void S3AccountUserIdxMetadata::save(std::function<void(void)> on_success, std::function<void(void)> on_failed) {
  s3_log(S3_LOG_DEBUG, "Entering\n");
  ...
  clovis_kv_writer->put_keyval(root_account_user_index_oid,
      get_account_user_index_name(), this->to_json(),
...
void S3AccountUserIdxMetadata::remove(std::function<void(void)> on_success, std::function<void(void)> on_failed) {
  s3_log(S3_LOG_DEBUG, "Entering\n");
  ...
  clovis_kv_writer = std::make_shared<S3ClovisKVSWriter>(request, s3_clovis_api);
  clovis_kv_writer->delete_keyval(root_account_user_index_oid, get_account_user_index_name(),
...
std::string S3AccountUserIdxMetadata::to_json() {
  ...
  root["bucket_list_index_oid_u_hi"] = base64_encode((unsigned char const*)&bucket_list_index_oid.u_hi, sizeof(bucket_list_index_oid.u_hi));
  root["bucket_list_index_oid_u_lo"] = base64_encode((unsigned char const*)&bucket_list_index_oid.u_lo, sizeof(bucket_list_index_oid.u_lo));
...
void S3AccountUserIdxMetadata::from_json(std::string content) {
  ...
  std::string bucket_list_index_oid_u_hi_str = base64_decode(root["bucket_list_index_oid_u_hi"].asString());
  std::string bucket_list_index_oid_u_lo_str = base64_decode(root["bucket_list_index_oid_u_lo"].asString());
