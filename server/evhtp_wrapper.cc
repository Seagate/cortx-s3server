...
int EvhtpWrapper::http_kvs_for_each(evhtp_kvs_t * kvs, evhtp_kvs_iterator cb, void * arg) {
  return evhtp_kvs_for_each(kvs, cb, arg);
}

const char * EvhtpWrapper::http_header_find(evhtp_headers_t * headers, const char * key) {
  return evhtp_header_find(headers, key);
}

void EvhtpWrapper::http_headers_add_header(evhtp_headers_t * headers, evhtp_header_t * header) {
...
void EvhtpWrapper::http_send_reply_start(evhtp_request_t * request, evhtp_res code) {
  evhtp_send_reply_start(request, code);
}

void EvhtpWrapper::http_send_reply_body(evhtp_request_t * request, evbuf_t * buf) {
