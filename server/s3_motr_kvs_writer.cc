...
void S3MotrKVSWriter::delete_index(struct m0_uint128 idx_oid,
  ...
  rc = s3_motr_api->clovis_entity_delete(&(idx_ctx->idx[0].in_entity),
                                         &(idx_op_ctx->ops[0]));
  if (rc != 0) {
    s3_log(S3_LOG_ERROR, request_id,
...
void S3MotrKVSWriter::delete_indexes(std::vector<struct m0_uint128> oids,
  ...
  for (size_t i = 0; i < ops_count; ++i) {
    ...
    rc = s3_motr_api->clovis_entity_delete(&(idx_ctx->idx[i].in_entity),
                                           &(idx_op_ctx->ops[i]));
    if (rc != 0) {
      s3_log(S3_LOG_ERROR, request_id,
             "clovis_entity_delete failed with return code: (%d)\n", rc);
...
void S3MotrKVSWriter::put_keyval(struct m0_uint128 oid, std::string key,
  ...
  rc = s3_motr_api->clovis_idx_op(&(idx_ctx->idx[0]), M0_IC_PUT, kvs_ctx->keys,
                                  kvs_ctx->values, kvs_ctx->rcs,
...
void S3MotrKVSWriter::delete_keyval(struct m0_uint128 oid,
  ...
  s3_motr_api->clovis_idx_init(&(idx_ctx->idx[0]), &motr_container.co_realm,
                               &oid_list[0]);
  idx_ctx->n_initialized_contexts = 1;
  rc = s3_motr_api->clovis_idx_op(&(idx_ctx->idx[0]), M0_IC_DEL, kvs_ctx->keys,
