...
class ConcreteMotrAPI : public MotrAPI {
 ...
 public:
  void motr_idx_init(struct m0_idx *idx, struct m0_realm *parent,
                     const struct m0_uint128 *id) {
    m0_idx_init(idx, parent, id);
  }

  void motr_obj_init(struct m0_obj *obj, struct m0_realm *parent,
  ...
  int motr_sync_op_init(struct m0_op **sync_op) {
  ...
  int motr_sync_entity_add(struct m0_op *sync_op, struct m0_entity *entity) {
  ...
  int motr_sync_op_add(struct m0_op *sync_op, struct m0_op *op) {
    if (is_motr_sync_should_be_faked()) {
      return 0;
    }
  ...
  int motr_entity_open(struct m0_entity *entity, struct m0_op **op) {
  ...
  int motr_entity_create(struct m0_entity *entity, struct m0_op **op) {
  ...
  int motr_entity_delete(struct m0_entity *entity, struct m0_op **op) {
  ...
  void motr_op_setup(struct m0_op *op, const struct m0_op_ops *ops,
                     m0_time_t linger) {
    m0_op_setup(op, ops, linger);
  }

  int motr_idx_op(struct m0_idx *idx, enum m0_idx_opcode opcode,
  ...
  void motr_op_launch(uint64_t addb_request_id, struct m0_op **op, uint32_t nr,
  ...
  int motr_op_wait(m0_op *op, uint64_t bits, m0_time_t op_wait_period) {
    return m0_op_wait(op, bits, op_wait_period);
  }

  int motr_op_rc(const struct m0_op *op) { return m0_rc(op); }
