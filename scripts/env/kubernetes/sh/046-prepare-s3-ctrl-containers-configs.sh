...
#kubectl apply -f k8s-blueprints/cortx-secret.yaml
#echo "cortx-secret applied!!!"
kubectl apply -f k8s-blueprints/shim-ctrl-pod.yaml

set +x
while [ `kubectl get pod | grep shim-ctrl-pod | grep  Running | wc -l` -lt 1 ]; do
  echo
  kubectl get pod | grep 'NAME\|shim-ctrl-pod'
  echo
