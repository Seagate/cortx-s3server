...
class ConfigCmd(SetupCmd):
  ...
  def configure_openldap_replication(self):
    ...
    storage_set_count = self.get_confvalue_with_validation('CONFIG>CONFSTORE_STORAGE_SET_COUNT_KEY')
      ...
      if len(server_nodes_list) > 1:
        ...
        with open(ldap_hosts_list_file, "w") as f:
          for node_machine_id in server_nodes_list:
            private_fqdn = self.get_confvalue_with_validation('CONFIG>CONFSTORE_PRIVATE_FQDN_KEY')
  ...
  def update_motr_max_units_per_request(self):
    ...
    motr_max_units_per_request = self.get_confvalue_with_validation('CONFIG>CONFSTORE_S3_MOTR_MAX_UNITS_PER_REQUEST')
