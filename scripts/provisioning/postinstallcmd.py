...
def update_motr_max_units_per_request(self):
    ...
    motr_max_units_per_request = self.get_confvalue(f'cortx>software>s3>motr_max_units_per_request')
    ...
    s3configfile = "/opt/seagate/cortx/s3/conf/s3config.yaml"
    if path.isfile(f'{s3configfile}') == False:
      ...
      raise S3PROVError(f'{s3configfile} file is not present')
        ...
        key = 'S3_MOTR_CONFIG>S3_MOTR_MAX_UNITS_PER_REQUEST'
        s3configfileconfstore = S3CortxConfStore(f'yaml://{s3configfile}', 'write_s3_motr_max_unit_idx')
        s3configfileconfstore.set_config(f'{key}', f'{motr_max_units_per_request}', True)
        self.logger.info(f'Key {key} updated successfully in {s3configfile}')
