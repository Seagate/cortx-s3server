...
usage() {
  ...
  echo '          --no-s3cortxutils-build    : Do not build s3cortxutils, Default (false)'
...
no-motrkvscli-build,no-s3background-build,no-s3cortxutils-build,\
...
while true; do
  case "$1" in
	...
	--no-s3cortxutils-build) no_s3cortxutils_build=1; shift ;;
