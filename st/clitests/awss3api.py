...
class AwsTest(S3PyCliTest):
    def __init__(self, description):
        os.environ["AWS_SHARED_CREDENTIALS_FILE"] = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), Config.aws_shared_credential_file)
        os.environ["AWS_CONFIG_FILE"] = os.path.join(os.path.dirname(
            os.path.realpath(__file__)), Config.aws_config_file)
        self.credentials = ""
    ...
    def setup(self):
        if hasattr(self, 'filename') and hasattr(self, 'filesize'):
            ...
            logit("Creating file [%s] with size [%d]" %
                  (file_to_create, self.filesize))
    ...
    def create_bucket(self, bucket_name, region=None, host=None):
        ...
        if region:
            ...
            self.with_cli("aws s3api" + " create-bucket " +
                          "--bucket " + bucket_name)
    ...
    def put_bucket_versioning(self, bucket_name, versioning_status):
        self.bucket_name = bucket_name
        self.with_cli("aws s3api " + " put-bucket-versioning " + "--bucket " + bucket_name
                      + " --versioning-configuration " + " Status=" + versioning_status)
        return self

    def get_bucket_versioning(self, bucket_name):
        self.bucket_name = bucket_name
        self.with_cli("aws s3api " + " get-bucket-versioning " +
                      "--bucket " + bucket_name)
        return self
    ...
    def list_bucket_tagging(self, bucket_name):
        self.bucket_name = bucket_name
        self.with_cli("aws s3api " + "get-bucket-tagging " +
                      "--bucket " + bucket_name)
        return self

    def delete_bucket_tagging(self, bucket_name):
        self.bucket_name = bucket_name
        self.with_cli("aws s3api " + "delete-bucket-tagging " +
                      "--bucket " + bucket_name)
        return self

    def delete_bucket(self, bucket_name):
        self.bucket_name = bucket_name
        self.with_cli("aws s3api " + " delete-bucket " +
                      "--bucket " + bucket_name)
        return self
    ...
    def list_object_tagging(self, bucket_name, object_name):
        self.bucket_name = bucket_name
        self.with_cli("aws s3api " + "get-object-tagging " +
    ...
    def delete_object_tagging(self, bucket_name, object_name):
        self.bucket_name = bucket_name
        self.with_cli("aws s3api " + "delete-object-tagging " +
                      "--bucket " + bucket_name + " --key " + object_name)
        return self

    def delete_object(self, bucket_name, object_name):
        self.bucket_name = bucket_name
        self.with_cli("aws s3api " + "delete-object " +
                      "--bucket " + bucket_name + " --key " + object_name)
    ...
    def delete_multiple_objects(self, bucket_name, object_list_file):
        self.bucket_name = bucket_name
        self.with_cli("aws s3api " + "delete-objects " + "--bucket " +
                      bucket_name + " --delete file://" + object_list_file)
        self.command = self.command + self.credentials
        return self
    ...
    def put_object(self, bucket_name, filename, filesize=None, canned_acl=None, debug_flag=None, key_name=None):
        ...
        if(key_name is not None):
            cmd = "aws s3api " + "put-object " + "--bucket " + \
    ...
    def upload_objects(self, bucket_name, root_dir_path="./tests-out", debug_flag=None):
        self.bucket_name = bucket_name
        self.filename = None
        cmd = "aws s3 cp {dir_path} s3://{bucket} --recursive".format(
            bucket=bucket_name, dir_path=root_dir_path)
        if(debug_flag is not None):
            cmd = cmd + " --debug"
        self.with_cli(cmd)
        return self
    ...
    def download_objects(self, bucket_name, root_dir_path="./tests-out", debug_flag=None):
        self.bucket_name = bucket_name
        self.filename = None
        cmd = "aws s3 cp s3://{bucket} {dir_path} --recursive".format(
            bucket=bucket_name, dir_path=root_dir_path)
        if(debug_flag is not None):
            cmd = cmd + " --debug"
        self.with_cli(cmd)
    ...
    def get_object_acl(self, bucket_name, object_name):
        ...
        self.with_cli("aws s3api " + "get-object-acl " + "--bucket " +
                      bucket_name + " --key " + object_name + " --output " + "json ")
        return self

    def get_bucket_acl(self, bucket_name):
        self.bucket_name = bucket_name
        self.with_cli("aws s3api " + "get-bucket-acl " +
    ...
    def put_object_with_permission_headers(self, bucket_name, object_name, permission_header, permission_header_value):
        cmd = "aws s3api " + "put-object " + "--bucket " + bucket_name + " --key " + \
            object_name + " --" + permission_header + " " + permission_header_value
        self.with_cli(cmd)
        return self
    ...
    def create_bucket_with_permission_headers(self, bucket_name, permission_header, permission_header_value):
        cmd = "aws s3api" + " create-bucket " + "--bucket " + bucket_name + \
            " --" + permission_header + " " + permission_header_value
        self.with_cli(cmd)
        return self

    def put_object_acl(self, bucket_name, object_name, permission_header, permission_header_value):
        cmd = "aws s3api " + "put-object-acl " + "--bucket " + bucket_name + " --key " + \
            object_name + " --" + permission_header + " " + permission_header_value
        self.with_cli(cmd)
        return self

    def put_object_acl_with_acp_file(self, bucket_name, object_name, acp):
        cmd = "aws s3api " + "put-object-acl " + "--bucket " + bucket_name + \
            " --key " + object_name + " --access-control-policy " + acp
        self.with_cli(cmd)
        return self
    ...
    def put_object_acl_with_canned_input(self, bucket_name, object_name, canned_input):
        cmd = "aws s3api " + "put-object-acl " + "--bucket " + \
            bucket_name + " --key " + object_name + " --acl " + canned_input
        self.with_cli(cmd)
        return self

    def put_bucket_acl(self, bucket_name, permission_header, permission_header_value):
        cmd = "aws s3api " + "put-bucket-acl " + "--bucket " + bucket_name + \
            " --" + permission_header + " " + permission_header_value
        self.with_cli(cmd)
        return self

    def put_bucket_acl_with_canned_input(self, bucket_name, canned_input):
        cmd = "aws s3api " + "put-bucket-acl " + "--bucket " + \
            bucket_name + " --acl " + canned_input
        self.with_cli(cmd)
        return self

    def put_bucket_acl_with_acp_file(self, bucket_name, acp):
        cmd = "aws s3api " + "put-bucket-acl " + "--bucket " + \
            bucket_name + " --access-control-policy " + acp
        self.with_cli(cmd)
        return self
    ...
    def get_object(self, bucket_name, object_name, outfile=None, debug_flag=None, start_range=None, end_range=None):
        ...
        if start_range is not None and end_range is not None:
            cmd = "aws s3api get-object --bucket " + bucket_name + " --key " + object_name + \
                " " + "--range bytes=" + start_range + "-" + end_range + " " + outfile
        else:
            cmd = "aws s3api get-object --bucket " + bucket_name + \
                " --key " + object_name + " " + outfile
        self.with_cli(cmd)
        if debug_flag is not None:
            self.command += " --debug"
        return self
    ...
    def head_object(self, bucket_name, object_name):
        self.bucket_name = bucket_name
        self.object_name = object_name
        self.with_cli("aws s3api " + "head-object " +
                      "--bucket " + bucket_name + " --key " + object_name)
        return self

    def head_bucket(self, bucket_name):
        self.bucket_name = bucket_name
        self.with_cli("aws s3api " + "head-bucket " +
                      "--bucket " + bucket_name)
        return self
