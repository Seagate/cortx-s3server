...
def load_test_config():
    conf_file = os.path.join(os.path.dirname(
        __file__), 's3iamcli_test_config.yaml')
...
# ******** Create Bucket ********
AwsTest('Aws can create bucket').create_bucket(
    "copyobjectbucket").execute_test().command_is_successful()

# creating dir for files.
upload_dir = os.path.join(os.path.dirname(
    os.path.realpath(__file__)), "copy_object_test_upload")
...
download_dir = os.path.join(os.path.dirname(
    os.path.realpath(__file__)), "copy_object_test_download")
...
# ******** Create Bucket ********
AwsTest('Aws can create bucket').create_bucket(
    "seagatebucket").execute_test().command_is_successful()
...
obj_list = []
for x in range(2):
    key = "loadgen_test_3XQEX2S6WFEDA4B32AJ3T6FAR4XVNKRAE6JET2LSJFFVACUV4MOMLCSH42RXFX22IXYWZS3YFFWYUJE4STNJPGFDOLIDLTQDZ6J2QLA_%d" % (
        x)
...
object_list_file = create_object_list_file(
    "obj_list_v2_keys.json", obj_list, "true")
...
temp_dir = os.path.join(os.path.dirname(
    os.path.realpath(__file__)), "s3testupload")
...
file_list = ['asdf', 'bo0', 'boo#', 'boo+', 'fo0',
             'foo#123', 'foo+123', 'qua0', 'quax#', 'quax+']
...
object_list_file = create_object_list_file(
    "obj_list_mix_keys.json", obj_list, "true")
...
#  quax/
prefix = ""
expected_key_list = ['asdf', 'bo0', 'boo#', 'boo+',
                     'fo0', 'foo#123', 'foo+123', 'qua0', 'quax#', 'quax+']
...
assert (("next_token" in lv2_response.keys()) and (len(
    lv2_response["next_token"].strip()) > 0)), "NextToken is either not present or empty"
...
#  boo+
list_v2_options = {"prefix": prefix, "delimiter": delimiter,
                   "max-items": 2, "starting-token": next_token}
result = AwsTest(('Aws list objects using prefix \'%s\' and delimiter %s') % (prefix, delimiter))\
    .list_objects_v2("seagatebucket", **list_v2_options)\
    .execute_test()\
    .command_is_successful()
...
# Create object list file
object_list_file = create_object_list_file(
    "obj_list_35_keys.json", obj_35_list, "true")
...
assert (("next_token" in list_response.keys()) and (len(
    list_response["next_token"].strip()) > 0)), "NextToken is either not present or empty"
...
AwsTest('Aws can create bucket').create_bucket(
    "rangereadbucket").execute_test().command_is_successful()

result = AwsTest('Aws can upload 10Mb file with tags').create_multipart_upload(
    "rangereadbucket", "10Mbfile", 10485760, "domain=storage").execute_test().command_is_successful()
upload_id = get_upload_id(result.status.stdout)
print(upload_id)

result = AwsTest('Aws can upload 5Mb first part').upload_part("rangereadbucket", "firstpart",
                                                              5242880, "10Mbfile", "1", upload_id).execute_test().command_is_successful()
e_tag_1 = result.status.stdout
print(e_tag_1)

result = AwsTest('Aws can upload 5Mb second part').upload_part("rangereadbucket",
                                                               "secondpart", 5242880, "10Mbfile", "2", upload_id).execute_test().command_is_successful()
e_tag_2 = result.status.stdout
print(e_tag_2)

parts = "Parts=[{ETag="+e_tag_1.strip('\n')+",PartNumber="+str(
    1)+"},{ETag="+e_tag_2.strip('\n')+",PartNumber="+str(2)+"}]"
print(parts)

result = AwsTest('Aws can complete multipart upload 10Mb file with tags').complete_multipart_upload("rangereadbucket", "10Mbfile",
                                                                                                    parts, upload_id).execute_test().command_is_successful().command_response_should_have("rangereadbucket/10Mbfile")
...
AwsTest('Aws can delete sourceobj').delete_object(
    "rangereadbucket", "10Mbfile").execute_test().command_is_successful()
...
AwsTest('Aws can create bucket').create_bucket(
    "simple-bucket").execute_test().command_is_successful()
...
# creating a bucket
AwsTest('Aws can create bucket').create_bucket(
    "multipart-bucket").execute_test().command_is_successful()

# Creating a first multipart upload
result = AwsTest('Aws can upload 10Mb file with tags').create_multipart_upload(
    "multipart-bucket", "10Mbfile1", 10485760, "domain=storage").execute_test().command_is_successful()
upload_id = get_upload_id(result.status.stdout)
print(upload_id)

result = AwsTest('Aws can upload 5Mb first part').upload_part("multipart-bucket", "firstpart",
                                                              5242880, "10Mbfile1", "1", upload_id).execute_test().command_is_successful()
e_tag_1 = result.status.stdout
print(e_tag_1)

result = AwsTest('Aws can upload 5Mb second part').upload_part("multipart-bucket",
                                                               "secondpart", 5242880, "10Mbfile1", "2", upload_id).execute_test().command_is_successful()
e_tag_2 = result.status.stdout
print(e_tag_2)
...
result = AwsTest('Aws can complete multipart upload 10Mb file with tags').complete_multipart_upload("multipart-bucket", "10Mbfile1",
                                                                                                    parts, upload_id).execute_test().command_is_successful().command_response_should_have("multipart-bucket/10Mbfile1")

# Creating a second multipart upload
result = AwsTest('Aws can upload 10Mb file with tags').create_multipart_upload(
    "multipart-bucket", "10Mbfile2", 10485760, "domain=storage").execute_test().command_is_successful()
upload_id = get_upload_id(result.status.stdout)
print(upload_id)

result = AwsTest('Aws can upload 5Mb first part').upload_part("multipart-bucket", "firstpart",
                                                              5242880, "10Mbfile2", "1", upload_id).execute_test().command_is_successful()
e_tag_1 = result.status.stdout
print(e_tag_1)

result = AwsTest('Aws can upload 5Mb second part').upload_part("multipart-bucket",
                                                               "secondpart", 5242880, "10Mbfile2", "2", upload_id).execute_test().command_is_successful()
e_tag_2 = result.status.stdout
print(e_tag_2)
...
result = AwsTest('Aws can complete multipart upload 10Mb file with tags').complete_multipart_upload("multipart-bucket", "10Mbfile2",
                                                                                                    parts, upload_id).execute_test().command_is_successful().command_response_should_have("multipart-bucket/10Mbfile2")
...
AwsTest('Aws can delete sourceobj').delete_object(
    "simple-bucket", "simpleobj1").execute_test().command_is_successful()

AwsTest('Aws can delete sourceobj').delete_object(
    "simple-bucket", "simpleobj2").execute_test().command_is_successful()

AwsTest('Aws can delete sourceobj').delete_object(
    "simple-bucket", "simpleobj3").execute_test().command_is_successful()
...
AwsTest('Aws can delete sourceobj').delete_object(
    "multipart-bucket", "10Mbfile1").execute_test().command_is_successful()

AwsTest('Aws can delete sourceobj').delete_object(
    "multipart-bucket", "10Mbfile2").execute_test().command_is_successful()
...
AwsTest('Aws can create bucket').create_bucket(
    "source-bucket").execute_test().command_is_successful()

# Aligned Copy
# ************** Create a multipart upload ********
result = AwsTest('Aws can upload 20Mb file with tags').create_multipart_upload(
    "source-bucket", "20Mbfile", 20971520, "domain=storage").execute_test().command_is_successful()
upload_id = get_upload_id(result.status.stdout)
print(upload_id)

# ************** Upload Individual parts ********
result = AwsTest('Aws can upload 10Mb first part').upload_part("source-bucket", "firstpart",
                                                               10485760, "20Mbfile", "1", upload_id).execute_test().command_is_successful()
e_tag_1 = result.status.stdout
print(e_tag_1)

result = AwsTest('Aws can upload 10Mb second part').upload_part("source-bucket", "secondpart",
                                                                10485760, "20Mbfile", "2", upload_id).execute_test().command_is_successful()
e_tag_2 = result.status.stdout
print(e_tag_2)
...
result = AwsTest('Aws can complete multipart upload 20Mb file with tags').complete_multipart_upload("source-bucket", "20Mbfile",
                                                                                                    parts, upload_id).execute_test().command_is_successful().command_response_should_have("source-bucket/20Mbfile")
...
dest_obj_data = AwsTest('Aws can get object').get_object(
    "destination-bucket", "20Mbfile-copy").execute_test().command_is_successful()
dest_obj_etag = get_etag(dest_obj_data.status.stdout)
print(dest_obj_etag)

source_obj_data = AwsTest('Aws can get object').get_object(
    "source-bucket", "20Mbfile-copy").execute_test().command_is_successful()
...
# *******************************UnAligned Copy********************************************
# ************** Create a multipart upload ********
result = AwsTest('Aws can upload 10.4Mb file with tags').create_multipart_upload(
    "source-bucket", "10.4Mbfile", 10905190, "domain=storage").execute_test().command_is_successful()
upload_id = get_upload_id(result.status.stdout)
print(upload_id)

# ************** Upload Individual parts ********
result = AwsTest('Aws can upload 5.2Mb first part').upload_part("source-bucket", "firstpart",
                                                                5452595, "10.4Mbfile", "1", upload_id).execute_test().command_is_successful()
e_tag_1 = result.status.stdout
print(e_tag_1)

result = AwsTest('Aws can upload 5.2Mb second part').upload_part("source-bucket", "secondpart",
                                                                 5452595, "10.4Mbfile", "2", upload_id).execute_test().command_is_successful()
e_tag_2 = result.status.stdout
print(e_tag_2)
...
# ************** Complete multipart upload ********
result = AwsTest('Aws can complete multipart upload 10.4Mb file with tags').complete_multipart_upload("source-bucket", "10.4Mbfile",
                                                                                                      parts, upload_id).execute_test().command_is_successful().command_response_should_have("source-bucket/10.4Mbfile")
...
# get-object after copying and validating Etag
dest_obj_data = AwsTest('Aws can get object').get_object(
    "destination-bucket", "10.4Mbfile-copy").execute_test().command_is_successful()
dest_obj_etag = get_etag(dest_obj_data.status.stdout)
print(dest_obj_etag)

source_obj_data = AwsTest('Aws can get object').get_object(
    "source-bucket", "10.4Mbfile-copy").execute_test().command_is_successful()
source_obj_etag = get_etag(source_obj_data.status.stdout)
print(source_obj_etag)
...
# ************** Create a multipart upload ********
result = AwsTest('Aws can upload 10Mb file with tags').create_multipart_upload(
    "source-bucket", "10MbAAfile", 10485760, "domain=storage").execute_test().command_is_successful()
upload_id = get_upload_id(result.status.stdout)
print(upload_id)

# ************** Upload Individual parts ********
result = AwsTest('Aws can upload 5Mb first part').upload_part("source-bucket", "firstpart",
                                                              5242880, "10MbAAfile", "1", upload_id).execute_test().command_is_successful()
e_tag_1 = result.status.stdout
print(e_tag_1)

result = AwsTest('Aws can upload 5Mb second part').upload_part("source-bucket", "secondpart",
                                                               5242880, "10MbAAfile", "2", upload_id).execute_test().command_is_successful()
e_tag_2 = result.status.stdout
print(e_tag_2)
...
# ************** Complete multipart upload ********
result = AwsTest('Aws can complete multipart upload 10Mb file with tags').complete_multipart_upload("source-bucket", "10MbAAfile",
                                                                                                    parts, upload_id).execute_test().command_is_successful().command_response_should_have("source-bucket/10MbAAfile")
...
AwsTest('Aws can get object').get_object("source-bucket",
                                         "10MbAAfile").execute_test().command_is_successful()
...
# ************** Create a multipart upload ********
result = AwsTest('Aws can upload 10Mb file with tags').create_multipart_upload(
    "source-bucket", "10MbABfile", 10485760, "domain=storage").execute_test().command_is_successful()
upload_id = get_upload_id(result.status.stdout)
print(upload_id)

# ************** Upload Individual parts ********
result = AwsTest('Aws can upload 5Mb first part').upload_part("source-bucket", "firstpart",
                                                              5242880, "10MbABfile", "1", upload_id).execute_test().command_is_successful()
e_tag_1 = result.status.stdout
print(e_tag_1)

result = AwsTest('Aws can upload 5Mb second part').upload_part("source-bucket", "secondpart",
                                                               5242880, "10MbABfile", "2", upload_id).execute_test().command_is_successful()
e_tag_2 = result.status.stdout
print(e_tag_2)
...
# ************** Complete multipart upload ********
result = AwsTest('Aws can complete multipart upload 10Mb file with tags').complete_multipart_upload("source-bucket", "10MbABfile",
                                                                                                    parts, upload_id).execute_test().command_is_successful().command_response_should_have("source-bucket/10MbABfile")

# get-object after overwrite
AwsTest('Aws can get object').get_object("source-bucket",
                                         "10MbABfile").execute_test().command_is_successful()

# CASE 3 : BA (overwrite multipart object with simple object)
# multipart object upload
# ************** Create a multipart upload ********
result = AwsTest('Aws can upload 10Mb file with tags').create_multipart_upload(
    "source-bucket", "10MbBAfile", 10485760, "domain=storage").execute_test().command_is_successful()
upload_id = get_upload_id(result.status.stdout)
print(upload_id)

# ************** Upload Individual parts ********
result = AwsTest('Aws can upload 5Mb first part').upload_part("source-bucket", "firstpart",
                                                              5242880, "10MbBAfile", "1", upload_id).execute_test().command_is_successful()
e_tag_1 = result.status.stdout
print(e_tag_1)

result = AwsTest('Aws can upload 5Mb second part').upload_part("source-bucket", "secondpart",
                                                               5242880, "10MbBAfile", "2", upload_id).execute_test().command_is_successful()
e_tag_2 = result.status.stdout
print(e_tag_2)
...
# ************** Complete multipart upload ********
result = AwsTest('Aws can complete multipart upload 10Mb file with tags').complete_multipart_upload("source-bucket", "10MbBAfile",
                                                                                                    parts, upload_id).execute_test().command_is_successful().command_response_should_have("source-bucket/10MbBAfile")
...
# get-object after overwrite
AwsTest('Aws can get object').get_object("source-bucket",
                                         "10MbBAfile").execute_test().command_is_successful()
...
# get-object after overwrite
AwsTest('Aws can get object').get_object("source-bucket",
                                         "10MbBBfile").execute_test().command_is_successful()

# Delete source and destination buckets and objects from them.
AwsTest('Aws can delete sourceobj').delete_object(
    "source-bucket", "10MbAAfile").execute_test().command_is_successful()

AwsTest('Aws can delete sourceobj').delete_object(
    "source-bucket", "10MbABfile").execute_test().command_is_successful()

AwsTest('Aws can delete sourceobj').delete_object(
    "source-bucket", "10MbBAfile").execute_test().command_is_successful()

AwsTest('Aws can delete sourceobj').delete_object(
    "source-bucket", "10MbBBfile").execute_test().command_is_successful()

AwsTest('Aws can delete sourceobj').delete_object(
    "source-bucket", "20Mbfile").execute_test().command_is_successful()

AwsTest('Aws can delete sourceobj').delete_object("source-bucket",
                                                  "20Mbfile-copy").execute_test().command_is_successful()

AwsTest('Aws can delete destinationobj').delete_object(
    "destination-bucket", "20Mbfile-copy").execute_test().command_is_successful()

AwsTest('Aws can delete sourceobj').delete_object(
    "source-bucket", "10.4Mbfile").execute_test().command_is_successful()

AwsTest('Aws can delete sourceobj').delete_object("source-bucket",
                                                  "10.4Mbfile-copy").execute_test().command_is_successful()

AwsTest('Aws can delete destinationobj').delete_object(
    "destination-bucket", "10.4Mbfile-copy").execute_test().command_is_successful()

AwsTest('Aws can delete source bucket').delete_bucket(
    "source-bucket").execute_test().command_is_successful()

AwsTest('Aws can delete destination bucket').delete_bucket(
    "destination-bucket").execute_test().command_is_successful()

# ************** Copy and overwrite ********************

# create source bucket
AwsTest('Aws can create bucket').create_bucket(
    "sourcebucket").execute_test().command_is_successful()

# ************** Create a multipart upload ********
result = AwsTest('Aws can upload 20Mb file with tags').create_multipart_upload(
    "sourcebucket", "20Mbfile", 20971520, "domain=storage").execute_test().command_is_successful()
upload_id = get_upload_id(result.status.stdout)
print(upload_id)

# ************** Upload Individual parts ********
result = AwsTest('Aws can upload 10Mb first part').upload_part("sourcebucket", "firstpart",
                                                               10485760, "20Mbfile", "1", upload_id).execute_test().command_is_successful()
e_tag_1 = result.status.stdout
print(e_tag_1)

result = AwsTest('Aws can upload 10Mb second part').upload_part("sourcebucket", "secondpart",
                                                                10485760, "20Mbfile", "2", upload_id).execute_test().command_is_successful()
e_tag_2 = result.status.stdout
print(e_tag_2)
...
# ************** Get object acl should fail before complete multipart upload ******
AwsTest('Aws can get object acl').get_object_acl("sourcebucket", "20Mbfile").execute_test(negative_case=True)\
    .command_should_fail().command_error_should_have("NoSuchKey")

# ************** Complete multipart upload ********
result = AwsTest('Aws can complete multipart upload 20Mb file with tags').complete_multipart_upload(
    "sourcebucket", "20Mbfile", parts, upload_id).execute_test().command_is_successful().command_response_should_have("sourcebucket/20Mbfile")

# Positive: copy multipart object to different destination bucket.
AwsTest('Aws can create destination bucket for Multipart CopyObject API')\
    .create_bucket("destinationbucket")\
    .execute_test().command_is_successful()

# copying to different bucket
AwsTest('Aws can copy object to different bucket')\
    .copy_object("sourcebucket/20Mbfile", "destinationbucket", "20Mbfile-copy")\
    .execute_test().command_is_successful().command_response_should_have("COPYOBJECTRESULT")

# copying to same bucket
AwsTest('Aws can copy object to same bucket')\
    .copy_object("sourcebucket/20Mbfile", "sourcebucket", "20Mbfile-copy")\
    .execute_test().command_is_successful().command_response_should_have("COPYOBJECTRESULT")

# overwrite copied objects with the simple and multipart objects
# overwrite in same bucket(positive case)
# with multipart object

# ************** Create a multipart upload ********
result = AwsTest('Aws can upload 10Mb file with tags').create_multipart_upload(
    "sourcebucket", "20Mbfile-copy", 10485760, "domain=storage").execute_test().command_is_successful()
upload_id = get_upload_id(result.status.stdout)
print(upload_id)

# ************** Upload Individual parts ********
result = AwsTest('Aws can upload 5Mb first part').upload_part("sourcebucket", "firstpart",
                                                              5242880, "20Mbfile-copy", "1", upload_id).execute_test().command_is_successful()
e_tag_1 = result.status.stdout
print(e_tag_1)

result = AwsTest('Aws can upload 5Mb second part').upload_part("sourcebucket", "secondpart",
                                                               5242880, "20Mbfile-copy", "2", upload_id).execute_test().command_is_successful()
e_tag_2 = result.status.stdout
print(e_tag_2)
...
# ************** Complete multipart upload ********
result = AwsTest('Aws can complete multipart upload 10Mb file with tags').complete_multipart_upload("sourcebucket", "20Mbfile-copy",
                                                                                                    parts, upload_id).execute_test().command_is_successful().command_response_should_have("sourcebucket/20Mbfile-copy")

# get-object after overwrite
AwsTest('Aws can get object').get_object("sourcebucket",
                                         "20Mbfile-copy").execute_test().command_is_successful()
...
# ************negative test case*****************
# overwrite in same bucket

obj_data = AwsTest('Aws can get object').get_object(
    "sourcebucket", "20Mbfile-copy").execute_test().command_is_successful()
dest_obj_etag = get_etag(obj_data.status.stdout)
print(dest_obj_etag)
...
AwsTest('Aws can abort previous multipart upload').abort_multipart_upload("sourcebucket",
                                                                          "20Mbfile-copy", upload_id).execute_test(negative_case=True).command_is_successful()

# get etag after aborting multipart upload
# both etags should be same, else test case is failed

obj_data_after_abort = AwsTest('Aws can get object').get_object(
    "sourcebucket", "20Mbfile-copy").execute_test().command_is_successful()
dest_obj_etag_after_abort = get_etag(obj_data_after_abort.status.stdout)
...
# overwrite in different bucket
# with multipart object

# ************** Create a multipart upload ********
result = AwsTest('Aws can upload 10Mb file with tags').create_multipart_upload(
    "destinationbucket", "20Mbfile-copy", 10485760, "domain=storage").execute_test().command_is_successful()
upload_id = get_upload_id(result.status.stdout)
print(upload_id)

# ************** Upload Individual parts ********
result = AwsTest('Aws can upload 5Mb first part').upload_part("destinationbucket", "firstpart",
                                                              5242880, "20Mbfile-copy", "1", upload_id).execute_test().command_is_successful()
e_tag_1 = result.status.stdout
print(e_tag_1)

result = AwsTest('Aws can upload 5Mb second part').upload_part("destinationbucket", "secondpart",
                                                               5242880, "20Mbfile-copy", "2", upload_id).execute_test().command_is_successful()
e_tag_2 = result.status.stdout
print(e_tag_2)
...
# ************** Complete multipart upload ********
result = AwsTest('Aws can complete multipart upload 10Mb file with tags').complete_multipart_upload("destinationbucket", "20Mbfile-copy",
                                                                                                    parts, upload_id).execute_test().command_is_successful().command_response_should_have("destinationbucket/20Mbfile-copy")

# get-object after overwrite
AwsTest('Aws can get object').get_object("destinationbucket",
                                         "20Mbfile-copy").execute_test().command_is_successful()

# with simple object
AwsTest('Aws can put object').put_object("destinationbucket", "20Mbfile-copy", 1024)\
    .execute_test().command_is_successful()
...
AwsTest('Aws can delete sourceobj').delete_object(
    "sourcebucket", "20Mbfile").execute_test().command_is_successful()

AwsTest('Aws can delete sourceobj').delete_object("sourcebucket",
                                                  "20Mbfile-copy").execute_test().command_is_successful()

AwsTest('Aws can delete destinationobj').delete_object(
    "destinationbucket", "20Mbfile-copy").execute_test().command_is_successful()

AwsTest('Aws can delete source bucket').delete_bucket(
    "sourcebucket").execute_test().command_is_successful()

AwsTest('Aws can delete destination bucket').delete_bucket(
    "destinationbucket").execute_test().command_is_successful()

# ************ CopyObject API supported *****************************************************************

AwsTest('Aws can create bucket').create_bucket(
    "sourcebucket").execute_test().command_is_successful()
AwsTest('Aws can create object with canned acl input').put_object("sourcebucket",
                                                                  "sourceobj", canned_acl="public-read").execute_test().command_is_successful()

test_msg = "Create account testAcc2"
account_args = {'AccountName': 'testAcc2', 'Email': 'testAcc2@seagate.com',
                'ldapuser': "sgiamadmin", 'ldappasswd': LdapInfo.get_ldap_admin_pwd()}
...
account_response_elements = AuthTest.get_response_elements(
...
AwsTest('Aws can create bucket').create_bucket(
    "destbucket").execute_test().command_is_successful()
...
AwsTest('CopyObject not accessible for allusers').execute_curl(
    cmd).execute_test().command_response_should_have("AccessDenied")
...
AwsTest('Aws can delete sourceobj').delete_object(
    "sourcebucket", "sourceobj").execute_test().command_is_successful()
AwsTest('Aws can delete sourcebucket').delete_bucket(
    "sourcebucket").execute_test().command_is_successful()
...
account_args = {'AccountName': 'testAcc2',
                'Email': 'testAcc2@seagate.com',  'force': True}
AuthTest('Delete account testAcc2').delete_account(
    **account_args).execute_test().command_response_should_have("Account deleted successfully")
...
obj_list = ['asdf', 'quax/t3/key.log', 'foo', 'quax/t1/key.log',
            'boo', 'zoo/p2/key.log', 'zoo/p7/key.log']
...
assert len(lv2_response["prefix"]) == 2, ((
    "Expecting common prefixes:{quax/, zoo/}\n. Actual:%s") % str((lv2_response["prefix"])))
assert len(lv2_response["keys"]) == 3, ((
    "Expecting keys:{asdf, boo, foo} in the output. Actual:%s") % str((lv2_response["keys"])))
...
assert len(lv2_response["keys"]) == 3, ((
    "Expected:[.a1, .f1, .p1]. Actual:%s") % str((lv2_response["keys"])))
...
common_prefix = ['test/.uds/.backup/',
                 'test/.uds/volumeGroupA/', 'test/.uds/volumeGroupB/']
...
assert (len(lv2_response["keys"]) == 21), "Expect 21 regular keys in the result, Actual = %d" % len(
    lv2_response["keys"])
...
# ************** Delete Bucket Tag ********
AwsTest('Aws can delete bucket tag').delete_bucket_tagging(
    "seagatebucket").execute_test().command_is_successful()
...
# ******* delete bucket **********
AwsTest('Aws can delete bucket').delete_bucket(
    "seagatebucket").execute_test().command_is_successful()

# ******** Create Bucket ********
AwsTest('Aws can create bucket').create_bucket(
    "seagatebuckettag").execute_test().command_is_successful()

# ******** Put Object Tag ********
AwsTest('Aws can upload 3k file with tags').put_object_with_tagging("seagatebuckettag",
                                                                    "3kfile", 3000, "organization=marketing").execute_test().command_is_successful()
...
AwsTest('Aws can update tags for 3k file').put_object_tagging("seagatebuckettag", "3kfile", [
    {'Key': 'seagate', 'Value': 'storage'}]).execute_test().command_is_successful()
...
# ************** Delete Object Tag ********
AwsTest('Aws can delete object tag').delete_object_tagging(
    "seagatebuckettag", "3kfile").execute_test().command_is_successful()
...
# ************** Delete Object  ********
AwsTest('Aws can delete object').delete_object(
    "seagatebuckettag", "3kfile").execute_test().command_is_successful()

# ************** Create a multipart upload ********
result = AwsTest('Aws can upload 10Mb file with tags').create_multipart_upload(
    "seagatebuckettag", "10Mbfile", 10485760, "domain=storage").execute_test().command_is_successful()
upload_id = get_upload_id(result.status.stdout)
print(upload_id)

# ************** Upload Individual parts ********
result = AwsTest('Aws can upload 5Mb first part').upload_part("seagatebuckettag",
                                                              "firstpart", 5242880, "10Mbfile", "1", upload_id).execute_test().command_is_successful()
e_tag_1 = result.status.stdout
print(e_tag_1)

result = AwsTest('Aws can upload 5Mb second part').upload_part("seagatebuckettag",
                                                               "secondpart", 5242880, "10Mbfile", "2", upload_id).execute_test().command_is_successful()
e_tag_2 = result.status.stdout
print(e_tag_2)
...
# ************** Complete multipart upload ********
result = AwsTest('Aws can complete multipart upload 10Mb file with tags').complete_multipart_upload("seagatebuckettag", "10Mbfile",
                                                                                                    parts, upload_id).execute_test().command_is_successful().command_response_should_have("seagatebuckettag/10Mbfile")

# ************* Get object ACL for multipart Upload **********

aclresult = AwsTest('Aws can get object acl').get_object_acl(
    "seagatebuckettag", "10Mbfile").execute_test().command_is_successful()

print("ACL validation started..")
AclTest('validate complete acl').validate_acl(
    aclresult, "C12345", "s3_test", "C12345", "s3_test", "FULL_CONTROL")
AclTest('acl has valid Owner').validate_owner(aclresult, "C12345", "s3_test")
AclTest('acl has valid Grants').validate_grant(
    aclresult, "C12345", "s3_test", 1, "FULL_CONTROL")
...
AwsTest('Aws can upload 1kfileA').put_object("seagatebuckettag",
                                             "1kfileA", 1000).execute_test().command_is_successful()
AwsTest('Aws can upload 1kfileB').put_object("seagatebuckettag",
                                             "1kfileB", 1000).execute_test().command_is_successful()
AwsTest('Aws can upload 1kfileC').put_object("seagatebuckettag",
                                             "1kfileC", 1000).execute_test().command_is_successful()

# **************** Create file with the above created objects **********************************
object_list_file = create_object_list_file("obj_list_file_all_exist.json", [
                                           "1kfileA", "1kfileB", "1kfileC"], "true")
...
AwsTest('Aws can upload 1kfileA').put_object("seagatebuckettag",
                                             "1kfileD", 1000).execute_test().command_is_successful()
AwsTest('Aws can upload 1kfileB').put_object("seagatebuckettag",
                                             "1kfileE", 1000).execute_test().command_is_successful()

# **************** Create file with the above created objects and one fake object **************
object_list_file = create_object_list_file("obj_list_file_not_all_exist.json", [
                                           "1kfileD", "1kfileE", "fakefileA"], "true")
...
object_list_file = create_object_list_file("obj_list_file_all_not_exist.json", [
                                           "fakefileB", "fakefileC", "fakefileD"], "true")
...
# *************** create 3 object keys in the bucket *******************************************
AwsTest('Aws can upload 1kfileF').put_object("seagatebuckettag",
                                             "1kfileF", 1000).execute_test().command_is_successful()
AwsTest('Aws can upload 1kfileG').put_object("seagatebuckettag",
                                             "1kfileG", 1000).execute_test().command_is_successful()
AwsTest('Aws can upload 1kfileH').put_object("seagatebuckettag",
                                             "1kfileH", 1000).execute_test().command_is_successful()

# **************** Create file with the above created objects **********************************
object_list_file = create_object_list_file("obj_list_file_all_exist_not_quiet_mode.json", [
                                           "1kfileF", "1kfileG", "1kfileH"], "false")
...
result = AwsTest('Multipart upload with wrong-sized parts').create_multipart_upload("seagatebuckettag",
                                                                                    "10Mbfile", 10485760, "domain=storage").execute_test().command_is_successful()
upload_id = get_upload_id(result.status.stdout)
print(upload_id)

result = AwsTest('Aws can upload 4Mb first part').upload_part("seagatebuckettag",
                                                              "firstpart", 4194304, "10Mbfile", "1", upload_id).execute_test().command_is_successful()
e_tag_1 = result.status.stdout
print(e_tag_1)

result = AwsTest('Aws can upload 4Mb second part').upload_part("seagatebuckettag",
                                                               "secondpart", 4194304, "10Mbfile", "2", upload_id).execute_test().command_is_successful()
e_tag_2 = result.status.stdout
print(e_tag_2)

result = AwsTest('Aws can upload 2Mb third part').upload_part("seagatebuckettag",
                                                              "thirdpart", 2097152, "10Mbfile", "3", upload_id).execute_test().command_is_successful()
...
result = AwsTest('Aws cannot complete multipart upload when non-last part is less than 5MB').complete_multipart_upload("seagatebuckettag",
                                                                                                                       "10Mbfile", parts, upload_id).execute_test(negative_case=True).command_should_fail().command_error_should_have("EntityTooSmall")
# Above test would not abort multipart operation, so need to do it explicitly
result = AwsTest('Aws abort previous multipart upload').abort_multipart_upload(
    "seagatebuckettag", "10Mbfile", upload_id).execute_test(negative_case=True).command_is_successful()

# ******* Multipart upload should fail if user completes multipart-upload with wrong ETag ******

result = AwsTest('Multipart upload with wrong ETag').create_multipart_upload(
    "seagatebuckettag", "10Mbfile", 10485760, "domain=storage").execute_test().command_is_successful()
upload_id = get_upload_id(result.status.stdout)
print(upload_id)
...
result = AwsTest('Aws cannot complete multipart upload with wrong ETag').complete_multipart_upload("seagatebuckettag", "10Mbfile",
                                                                                                   parts, upload_id).execute_test(negative_case=True).command_should_fail().command_error_should_have("InvalidPart")
...
account_response_pattern = "AccountId = [\w-]*, CanonicalId = [\w-]*, RootUserName = [\w+=,.@-]*, AccessKeyId = [\w-]*, SecretKey = [\w/+]*$"
result1 = AuthTest(test_msg).create_account(
    **source_account_args).execute_test()
...
account_response_pattern = "AccountId = [\w-]*, CanonicalId = [\w-]*, RootUserName = [\w+=,.@-]*, AccessKeyId = [\w-]*, SecretKey = [\w/+]*$"
result1 = AuthTest(test_msg).create_account(
    **target_account_args).execute_test()
result1.command_should_match_pattern(account_response_pattern)
account_response_elements = get_response_elements(result1.status.stdout)
...
account_response_pattern = "AccountId = [\w-]*, CanonicalId = [\w-]*, RootUserName = [\w+=,.@-]*, AccessKeyId = [\w-]*, SecretKey = [\w/+]*$"
result1 = AuthTest(test_msg).create_account(
    **cross_account_args).execute_test()
result1.command_should_match_pattern(account_response_pattern)
account_response_elements = get_response_elements(result1.status.stdout)
...
AwsTest('Aws can create bucket').create_bucket(
    "target-bucket").execute_test().command_is_successful()
...
os.environ["AWS_ACCESS_KEY_ID"] = source_access_key_args['AccessKeyId']
os.environ["AWS_SECRET_ACCESS_KEY"] = source_access_key_args['SecretAccessKey']
policy_source_bucket_relative = os.path.join(
    os.path.dirname(__file__), 'policy_files', 'source_bucket.json')
policy_source_bucket = "file://" + \
    os.path.abspath(policy_source_bucket_relative)
AwsTest("Aws can put policy on bucket").put_bucket_policy(
    "source-bucket", policy_source_bucket).execute_test().command_is_successful()
os.environ["AWS_ACCESS_KEY_ID"] = cross_access_key_args['AccessKeyId']
os.environ["AWS_SECRET_ACCESS_KEY"] = cross_access_key_args['SecretAccessKey']
...
os.environ["AWS_ACCESS_KEY_ID"] = destination_access_key_args['AccessKeyId']
os.environ["AWS_SECRET_ACCESS_KEY"] = destination_access_key_args['SecretAccessKey']
policy_target_bucket_relative = os.path.join(
    os.path.dirname(__file__), 'policy_files', 'target_bucket.json')
policy_target_bucket = "file://" + \
    os.path.abspath(policy_target_bucket_relative)
AwsTest("Aws can put policy on bucket").put_bucket_policy(
    "target-bucket", policy_target_bucket).execute_test().command_is_successful()
os.environ["AWS_ACCESS_KEY_ID"] = cross_access_key_args['AccessKeyId']
os.environ["AWS_SECRET_ACCESS_KEY"] = cross_access_key_args['SecretAccessKey']
...
policy_target_bucket_relative_new = os.path.join(os.path.dirname(
    __file__), 'policy_files', 'target_bucket_putobjecttagging.json')
policy_target_bucket_new = "file://" + \
    os.path.abspath(policy_target_bucket_relative_new)
...
policy_target_bucket_relative_acl = os.path.join(os.path.dirname(
    __file__), 'policy_files', 'target_bucket_denyobjectacl.json')
policy_target_bucket_acl = "file://" + \
    os.path.abspath(policy_target_bucket_relative_acl)
...
os.environ["AWS_ACCESS_KEY_ID"] = source_access_key_args['AccessKeyId']
os.environ["AWS_SECRET_ACCESS_KEY"] = source_access_key_args['SecretAccessKey']
AwsTest("Aws can delete policy on bucket").delete_bucket_policy(
    "source-bucket").execute_test().command_is_successful()
os.environ["AWS_ACCESS_KEY_ID"] = destination_access_key_args['AccessKeyId']
os.environ["AWS_SECRET_ACCESS_KEY"] = destination_access_key_args['SecretAccessKey']
# Copy Object should fail now
...
AwsTest('Aws can put bucket acl').put_bucket_acl("target-bucket", "grant-write",
                                                 "id=" + cross_access_key_args['CanonicalId']).execute_test().command_is_successful()
os.environ["AWS_ACCESS_KEY_ID"] = cross_access_key_args['AccessKeyId']
os.environ["AWS_SECRET_ACCESS_KEY"] = cross_access_key_args['SecretAccessKey']
AwsTest('Aws can copy object').copy_object("source-bucket/source-object", "target-bucket", "source-object")\
    .execute_test().command_is_successful()
...
AwsTest('Aws can get bucket versioning default status').get_bucket_versioning(
    "seagatebucket").execute_test().command_response_should_be_empty()

# ******** Enable Versioning on Bucket ********
AwsTest('Aws can enable versioning on bucket').put_bucket_versioning(
    "seagatebucket", "Enabled").execute_test().command_is_successful()

# ******** Get Bucket Versioning Enabled status ********
AwsTest('Aws can get bucket versioning Enabled status').get_bucket_versioning(
    "seagatebucket").execute_test().command_response_should_have("Enabled")

# ******** Suspend Versioning on Bucket ********
AwsTest('Aws can suspend versioning on bucket').put_bucket_versioning(
    "seagatebucket", "Suspended").execute_test().command_is_successful()

# ******** Get Bucket Versioning Suspended status ********
AwsTest('Aws can get bucket versioning Suspended status').get_bucket_versioning(
    "seagatebucket").execute_test().command_response_should_have("Suspended")
...
account_args = {'AccountName': 'sourceAccount',
                'Email': 'sourceAccount@seagate.com',  'force': True}
...
account_args = {'AccountName': 'destinationAccount',
                'Email': 'destinationAccount@seagate.com',  'force': True}
AuthTest(test_msg).delete_account(**target_account_args).execute_test()\
    .command_response_should_have("Account deleted successfully")

test_msg = "Delete account crossAccount"
account_args = {'AccountName': 'crossAccount',
                'Email': 'crossAccount@seagate.com',  'force': True}
