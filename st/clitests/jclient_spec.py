...
for i, val in enumerate(pathstyle_values):
    ...
    JClientTest('Jclient can upload 18MB file (multipart)').put_object_multipart("seagatebucket", "18MBfile", 18000000, 5)\
            .execute_test().command_is_successful()

    JClientTest('Jclient can get object acl').get_acl("seagatebucket", "18MBfile").execute_test().command_is_successful().command_response_should_have('C12345: FULL_CONTROL')

    JClientTest('Jclient cannot upload 18MB file (multipart) to nonexistent bucket').put_object_multipart("seagate-bucket", "18MBfile", 18000000, 5)\
    ...
    JClientTest('Jclient can upload 18MB file (multipart) in chunked mode').put_object_multipart("seagatebucket", "18MBfilec", 18000000, 5, chunked=True)\
            .execute_test().command_is_successful()
    ...
    JClientTest('Jclient can upload tes@t file with 18MB (multipart)').put_object_multipart("seagate.bucket", "tes@t", 18000000, 5)\
            .execute_test().command_is_successful()
    ...
    JClientTest('JClient can upload other parts before uploading part-1')\
