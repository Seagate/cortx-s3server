...
public class AuthEncryptCLI {
	...
	public static String AUTH_INSTALL_DIR = "/opt/seagate/cortx/auth";
	...
	public static void usage() {
		System.out.println("Usage: java -jar AuthPassEncryptCLI-1.0-0.jar" + " [options]");
	 ...
	 * @return CLIArgs
	 */
	public static CLIArgs readFromConsole() {
		...
		char[] tmp3 = console.readPassword("Enter encryption algorithm to be used:");
		...
		if (tmp3.length == 0) {
			...
			if (!EncryptionAlgorithm.isEncryptionAlgorithmValid(encryptionAlgorithm)) {
	...
	public static CLIArgs parseArgs(String[] args) {
		...
		if (args.length > 0) {
			...
			} else if ((args.length == 4) && (args[0].equals("-s")) && (args[2].equals("-e"))) {
	 ...
	 * @throws Exception
	 */
	public static String processEncryptRequest(String passwd) throws GeneralSecurityException, Exception {
		...
		PublicKey pKey = JKSUtil.getPublicKeyFromJKS(s3KeystoreFile.toString(), certAlias, keystorePasswd);
	 ...
	 * @throws Exception
	 */
	public static String processEncryptByAesRequest(String passwd) throws Exception {
		String encryptedPasswd = null;
		if (passwd == null || passwd.isEmpty() || passwd.matches(".*([ \t]).*")) {
			logger.error("Password is null or empty or contains spaces");
			System.err.println("Invalid Password Value.");
	...
	private static String getKey(String cipherUtilCmd) {
		String key = null;
		try {
			...
			BufferedReader reader = new BufferedReader(new InputStreamReader(s3Cipher.getInputStream()));
			...
			logger.debug(e.getMessage() + " IO error in S3 cipher. Loading default keystore credentilas.");
	...
	static void logInit() throws CLIInitializationException, Exception {
		...
		/**
		 * If log4j config file is given, override the default Logging properties file.
		 */
		if (logConfigFilePath != null) {
			...
			if (logConfigFile.exists()) {
				...
				throw new CLIInitializationException("Logging config file " + logConfigFilePath + " doesn't exist.");
			}
		}
		...
		if (logLevel != null) {
			...
			if (level == null) {
				...
				throw new Exception("Invalid log value [" + logLevel + "]" + "specified in config file");
			} else {
	...
	public static void main(String[] args) {
		...
		try {
			AuthEncryptConfig.readConfig(AUTH_INSTALL_DIR + "/resources/keystore.properties");
		} catch (IOException e1) {
			logger.error("Failed to read Properties from install dir:" + AUTH_INSTALL_DIR + " Error Stack :"
					+ e1.getStackTrace());
			System.err.println("Failed to read Properties from install dir:" + AUTH_INSTALL_DIR);
			System.exit(1);
		}
		try {
			logInit();
		...
		try {
			...
			logger.error("Failed to encrypt password. " + e.getCause() + e.getMessage());
