...
@PrepareForTest({ LDAPUtils.class, AccessKeyImpl.class, AuthServerConfig.class, LDAPConnection.class,
		LdapConnectionManager.class, FaultPoints.class, AuthServerConfig.class, AESEncryptDecryptUtil.class })
@PowerMockIgnore({ "javax.management.*" })
public class AccessKeyImplTest {

	private final String ACCESSKEY_BASE_DN = "ou=accesskeys,dc=s3,dc=seagate," + "dc=com";
	private final String[] FIND_ATTRS = { "s3userid", "sk", "exp", "token", "status", "createtimestamp",
			"objectclass" };

	private final String[] FIND_ALL_ATTRS = { "ak", "status", "createtimestamp", "token" };

	private final String[] GETCOUNT_ATTRS = { "ak", "exp" };
	...
	private void setupAccessKeyAttr() throws Exception {
		...
		PowerMockito.when(AESEncryptDecryptUtil.class, "decrypt", Mockito.any(String.class), Mockito.any(String.class))
				.thenReturn("decrypted-sk-123/test");
		Mockito.when(entry.getAttribute("sk")).thenReturn(secretKeyAttr);
		Mockito.when(secretKeyAttr.getStringValue()).thenReturn("encrypted-sk-123/test");

		Mockito.when(entry.getAttribute("status")).thenReturn(statusAttr);
		Mockito.when(statusAttr.getStringValue()).thenReturn("Active");

		Mockito.when(entry.getAttribute("createtimestamp")).thenReturn(createTimeStampAttr);
		Mockito.when(createTimeStampAttr.getStringValue()).thenReturn(LDAP_DATE);

		Mockito.when(entry.getAttribute("objectclass")).thenReturn(objectClassAttr);
	...
	public void setUp() throws Exception {
		...
		PowerMockito.mockStatic(AESEncryptDecryptUtil.class);
		...
		PowerMockito.when(ldapConnection.isConnected()).thenReturn(Boolean.TRUE);
		PowerMockito.mockStatic(LdapConnectionManager.class);
		PowerMockito.doReturn(ldapConnection).when(LdapConnectionManager.class, "getConnection");
		PowerMockito.whenNew(LDAPConnection.class).withArguments(socket_timeout).thenReturn(ldapConnection);
	}
	...
	public void Find_AccessKeySearchFailed_ThrowException() throws Exception {
		...
		PowerMockito.doThrow(new LDAPException()).when(ldapConnection).search(ACCESSKEY_BASE_DN, 2, filter, FIND_ATTRS,
				false);
	...
	public void Find_AccessKey_ThrowException() throws Exception {
		String filter = "ak=AKIATEST";
		PowerMockito.doReturn(ldapResults).when(ldapConnection).search(ACCESSKEY_BASE_DN, 2, filter, FIND_ATTRS, false);
	...
	@Test
	public void findAccessKeyFoundReturnAccessKey() throws Exception {
		AccessKey expectedAccessKey = new AccessKey();
		expectedAccessKey.setId("AKIATEST");
		expectedAccessKey.setUserId("123");
		expectedAccessKey.setSecretKey("decrypted-sk-123/test");
		...
		PowerMockito.doReturn(ldapResults).when(LDAPUtils.class, "search", ACCESSKEY_BASE_DN, 2, filter, FIND_ATTRS);
		PowerMockito.doReturn(ldapResults).when(ldapConnection).search(ACCESSKEY_BASE_DN, 2, filter, FIND_ATTRS, false);
		Mockito.when(ldapResults.hasMore()).thenReturn(Boolean.TRUE).thenReturn(Boolean.FALSE);
	...
	@Test
	public void findFedAccessKeyFoundReturnAccessKey() throws Exception {
		AccessKey expectedAccessKey = new AccessKey();
		expectedAccessKey.setId("AKIATEST");
		expectedAccessKey.setUserId("123");
		expectedAccessKey.setSecretKey("decrypted-sk-123/test");
		...
		Mockito.when(entry.getAttribute("exp")).thenReturn(expiryAttr);
		Mockito.when(expiryAttr.getStringValue()).thenReturn("20160129160752Z");
		Mockito.when(entry.getAttribute("token")).thenReturn(tokenAttr);
	...
	@Test
	public void FindAll_AccessKeySearchFailed_ThrowException() throws Exception {
		User user = new User();
		user.setId("123");
		String filter = "(&(s3userid=123)(objectclass=accesskey))";

		PowerMockito.doThrow(new LDAPException()).when(LDAPUtils.class, "search", ACCESSKEY_BASE_DN, 2, filter,
				FIND_ALL_ATTRS);
	...
	public void FindAll_AccessKeyEmpty_ReturnEmptyList() throws Exception {
		...
		PowerMockito.doReturn(ldapResults).when(LDAPUtils.class, "search", ACCESSKEY_BASE_DN, 2, filter,
				FIND_ALL_ATTRS);
		Mockito.when(ldapResults.hasMore()).thenReturn(Boolean.FALSE);

		AccessKey[] accessKeyList = accesskeyImpl.findAll(user);
		Assert.assertThat(expectedAccessKeyList, new ReflectionEquals(accessKeyList));
	}

	@Test
	public void FindAll_AccessKeyIterationFailed_ThrowException() throws Exception {
		User user = new User();
		user.setId("123");
	...
	public void FindAll_AccessKeyFound_ReturnList() throws Exception {
		...
		Assert.assertThat(expectedAccessKey, new ReflectionEquals(accessKeyList[0]));
	}

	@Test
	public void GetCount_AccessKeySearchFailed_ThrowException() throws Exception {
		String filter = "(&(s3userid=123)(objectclass=accesskey))";
		PowerMockito.doThrow(new LDAPException()).when(LDAPUtils.class, "search", ACCESSKEY_BASE_DN, 2, filter,
				GETCOUNT_ATTRS);
	...
	@Test
	public void GetCount_ExceptionOccuredWhileIterating_ThrowException() throws Exception {
		String filter = "(&(s3userid=123)(objectclass=accesskey))";
		exception.expect(DataAccessException.class);
	...
	public void GetCount_HasTwoAccessKeys_ReturnTwo() throws Exception {
		...
		PowerMockito.doReturn(ldapResults).when(LDAPUtils.class, "search", ACCESSKEY_BASE_DN, 2, filter,
				GETCOUNT_ATTRS);
		Mockito.when(ldapResults.hasMore()).thenReturn(Boolean.TRUE).thenReturn(Boolean.TRUE).thenReturn(Boolean.FALSE);
	...
	public void Delete_AccessKeyDeleteFailed_ThrowException() throws Exception {
		...
		PowerMockito.doThrow(new LDAPException()).when(LDAPUtils.class, "delete", dn);
	...
	@Test
	public void saveAccessKeySaveFailedThrowException() throws Exception {
		AccessKey accessKey = new AccessKey();
		...
		PowerMockito.doThrow(new LDAPException()).when(LDAPUtils.class, "add", Matchers.any(LDAPEntry.class));

		PowerMockito.when(AESEncryptDecryptUtil.class, "encrypt", Mockito.any(String.class), Mockito.any(String.class))
						.thenReturn("encrypted-s3-1232/test");
	...
	@Test
	public void saveAccessKeySaved() throws Exception {
		AccessKey accessKey = new AccessKey();
		accessKey.setUserId("123");
		accessKey.setId("AKIATEST");
		accessKey.setSecretKey("s3-1232/test");
		...
		PowerMockito.whenNew(LDAPAttributeSet.class).withNoArguments().thenReturn(ldapAttributeSet);

		PowerMockito.whenNew(LDAPAttribute.class).withAnyArguments().thenReturn(ldapAttribute);

		Mockito.doReturn(true).when(ldapAttributeSet).add(ldapAttribute);
		PowerMockito.whenNew(LDAPEntry.class).withParameterTypes(String.class, LDAPAttributeSet.class)
				.withArguments(dn, ldapAttributeSet).thenReturn(accessKeyEntry);
		...
		PowerMockito.verifyNew(LDAPAttribute.class).withArguments("objectclass", "accesskey");
		PowerMockito.verifyNew(LDAPAttribute.class).withArguments("s3userid", "123");
		PowerMockito.verifyNew(LDAPAttribute.class).withArguments("ak", "AKIATEST");
		PowerMockito.verifyNew(LDAPAttribute.class).withArguments("sk", "encrypted-s3-1232/test");
		PowerMockito.verifyNew(LDAPAttribute.class).withArguments("status", "Active");
	...
	@Test
	public void saveFedAccessKeySaveFailedThrowException() throws Exception {
		AccessKey accessKey = new AccessKey();
		accessKey.setUserId("123");
		accessKey.setId("AKIATEST");
		accessKey.setSecretKey("s3-1232/test");
	...
	@Test
	public void saveFedAccessKeySaved() throws Exception {
		AccessKey accessKey = new AccessKey();
		accessKey.setUserId("123");
		accessKey.setId("AKIATEST");
		accessKey.setSecretKey("s3-1232/test");
		...
		PowerMockito.verifyNew(LDAPAttribute.class).withArguments("objectclass", "fedaccessKey");
		PowerMockito.verifyNew(LDAPAttribute.class).withArguments("s3userid", "123");
		PowerMockito.verifyNew(LDAPAttribute.class).withArguments("ak", "AKIATEST");
		PowerMockito.verifyNew(LDAPAttribute.class).withArguments("sk", "encrypted-s3-1232/test");
		PowerMockito.verifyNew(LDAPAttribute.class).withArguments("token", "s3token/812-12");
		PowerMockito.verifyNew(LDAPAttribute.class).withArguments("exp", "20160109105536Z");
		PowerMockito.verifyNew(LDAPAttribute.class).withArguments("status", "Active");
	...
	public void Update_AccessKeyModifyFailed_ThrowException() throws Exception {
		...
		PowerMockito.whenNew(LDAPAttribute.class).withParameterTypes(String.class, String.class)
				.withArguments("status", "Active").thenReturn(updateAttr);

		PowerMockito.whenNew(LDAPModification.class).withParameterTypes(int.class, LDAPAttribute.class)
				.withArguments(LDAPModification.REPLACE, updateAttr).thenReturn(modification);

		String dn = "ak=AKIATEST,ou=accesskeys,dc=s3,dc=seagate,dc=com";
		PowerMockito.doThrow(new LDAPException()).when(LDAPUtils.class, "modify", dn, modification);
	...
	public void UpdateUser_UserModifySuccess() throws Exception {
		...
		String dn = "ak=AKIATEST,ou=accesskeys,dc=s3,dc=seagate,dc=com";
		PowerMockito.doNothing().when(LDAPUtils.class, "modify", dn, modification);
	...
	@Test
	public void findFromTokenAccessKeyFoundReturnAccessKey() throws Exception {
		AccessKey expectedAccessKey = new AccessKey();
		expectedAccessKey.setUserId("123");
		expectedAccessKey.setSecretKey("decrypted-sk-123/test");
		expectedAccessKey.setCreateDate(EXPECTED_DATE);
		...
		String[] attrs = { LDAPUtils.USER_ID, LDAPUtils.SECRET_KEY, LDAPUtils.EXPIRY, LDAPUtils.ACCESS_KEY_ID,
				LDAPUtils.STATUS, LDAPUtils.CREATE_TIMESTAMP, LDAPUtils.OBJECT_CLASS };

		setupAccessKeyAttr();

		PowerMockito.doReturn(ldapResults).when(LDAPUtils.class, "search", ACCESSKEY_BASE_DN, 2, filter, attrs);
		Mockito.when(ldapResults.hasMore()).thenReturn(Boolean.TRUE).thenReturn(Boolean.FALSE);
	...
	public void FindFromToken_SearchShouldTrowException() throws Exception {
		...
		PowerMockito.doThrow(new LDAPException()).when(LDAPUtils.class, "search", ACCESSKEY_BASE_DN, 2, filter, attrs);
	...
	@Test
	public void findFromTokenFedAccessKeyFoundReturnAccessKey() throws Exception {
		AccessKey expectedAccessKey = new AccessKey();
		expectedAccessKey.setId("AKIATEST");
		expectedAccessKey.setUserId("123");
		expectedAccessKey.setSecretKey("decrypted-sk-123/test");
	...
	@Test
	public void findAccountAccessKeySuccess() throws Exception {
		User user = new User();
		user.setId("123");
		String filter = "(&(s3userid=123)(objectclass=accesskey))";
		String[] attrs = { LDAPUtils.ACCESS_KEY_ID, LDAPUtils.SECRET_KEY, LDAPUtils.STATUS, LDAPUtils.EXPIRY,
				LDAPUtils.OBJECT_CLASS, LDAPUtils.CREATE_TIMESTAMP, LDAPUtils.TOKEN, LDAPUtils.USER_ID };
		AccessKey expectedAccessKey = new AccessKey();
		expectedAccessKey.setId("AKIATEST");
		expectedAccessKey.setUserId("123");
		expectedAccessKey.setSecretKey("decrypted-sk-123/test");
	...
	@Test
	public void findAccountAccessKey_EmptyAccessKey() throws Exception {
		String filter = "(&(s3userid=123)(objectclass=accesskey))";
		String[] attrs = { LDAPUtils.ACCESS_KEY_ID, LDAPUtils.SECRET_KEY, LDAPUtils.STATUS, LDAPUtils.EXPIRY,
				LDAPUtils.OBJECT_CLASS, LDAPUtils.CREATE_TIMESTAMP, LDAPUtils.TOKEN, LDAPUtils.USER_ID };
	...
	@Test(expected = DataAccessException.class)
	public void findAccountAccessKey_SearchFailedThrowsException() throws Exception {
		String filter = "(&(s3userid=123)(objectclass=accesskey))";
		String[] attrs = { LDAPUtils.ACCESS_KEY_ID, LDAPUtils.SECRET_KEY, LDAPUtils.STATUS, LDAPUtils.EXPIRY,
				LDAPUtils.OBJECT_CLASS, LDAPUtils.CREATE_TIMESTAMP, LDAPUtils.TOKEN, LDAPUtils.USER_ID };
	...
	@Test(expected = DataAccessException.class)
	public void findAccountAccessKey_SearchNextFailedThrowsException() throws Exception {
		String filter = "(&(s3userid=123)(objectclass=accesskey))";
		String[] attrs = { LDAPUtils.ACCESS_KEY_ID, LDAPUtils.SECRET_KEY, LDAPUtils.STATUS, LDAPUtils.EXPIRY,
				LDAPUtils.OBJECT_CLASS, LDAPUtils.CREATE_TIMESTAMP, LDAPUtils.TOKEN, LDAPUtils.USER_ID };
