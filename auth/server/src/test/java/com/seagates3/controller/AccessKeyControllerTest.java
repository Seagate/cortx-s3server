...
public class AccessKeyControllerTest {
   ...
   private
    ...
    public void setUp() throws Exception {
        ...
        PowerMockito.doReturn(ACCESS_KEY_ID)
    ...
    @Test public void createAccessKeyUserCred() throws Exception {
      ...
      Mockito.doReturn(user).when(userDAO).find(ACCOUNT_NAME, REQUESTOR_NAME);
      Mockito.doReturn(existingAccessKey).when(accessKeyDAO).find(
          Mockito.anyString());
      Mockito.doReturn(0).when(accessKeyDAO).getCount("123");
      Mockito.doNothing().when(accessKeyDAO).save(Mockito.any(AccessKey.class));
      ServerResponse response = accessKeyController.create();
    ...
    @Test public void createAccessKeyUserCredAccessKeyException()
      ...
      Mockito.doReturn(user).when(userDAO).find(ACCOUNT_NAME, REQUESTOR_NAME);
      Mockito.doThrow(DataAccessException.class).when(accessKeyDAO).find(
          Mockito.anyString());
      Mockito.doReturn(0).when(accessKeyDAO).getCount("123");
      Mockito.doNothing().when(accessKeyDAO).save(Mockito.any(AccessKey.class));
      ServerResponse response = accessKeyController.create();
