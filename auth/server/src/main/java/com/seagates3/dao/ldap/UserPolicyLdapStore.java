...
public class UserPolicyLdapStore {

	private final Logger LOGGER = LoggerFactory.getLogger(UserPolicyLdapStore.class.getName());

	public void attach(Object userPolicyObj) throws DataAccessException {
		...
		String dn = String.format("%s=%s,%s=%s,%s=%s,%s=%s,%s", LDAPUtils.USER_ID, user.getId(),
				LDAPUtils.ORGANIZATIONAL_UNIT_NAME, LDAPUtils.USER_OU, LDAPUtils.ORGANIZATIONAL_NAME,
				user.getAccountName(), LDAPUtils.ORGANIZATIONAL_UNIT_NAME, LDAPUtils.ACCOUNT_OU, LDAPUtils.BASE_DN);
		...
		LOGGER.debug("Updating user dn: " + dn + " user name: " + user.getName() + " with policy id: "
				+ policy.getPolicyId());
	...
	public void detach(Object userPolicyObj) throws DataAccessException {
		UserPolicy userPolicy = (UserPolicy) userPolicyObj;
		User user = userPolicy.getUser();
		Policy policy = userPolicy.getPolicy();
