...
class LdapStore implements AuthStore {
 ...
 private
  ...
  @Override public Policy find(String policyarn) throws DataAccessException {
    ...
    String ldapBase =
        String.format("%s=%s,%s", LDAPUtils.ORGANIZATIONAL_UNIT_NAME,
                      LDAPUtils.POLICY_OU, LDAPUtils.BASE_DN);
    ...
    if (ldapResults != null && ldapResults.hasMore()) {
      try {
        ...
        String modifyTimeStamp =
            entry.getAttribute(LDAPUtils.MODIFY_TIMESTAMP).getStringValue();
  ...
  @Override public List<Policy> findAll(Account account)
    ...
    String ldapBase =
        String.format("%s=%s,%s=%s,%s", LDAPUtils.ORGANIZATIONAL_NAME,
                      account.getName(), LDAPUtils.ORGANIZATIONAL_UNIT_NAME,
                      LDAPUtils.ACCOUNT_OU, LDAPUtils.BASE_DN);
    String filter = String.format("(%s=%s)", LDAPUtils.ORGANIZATIONAL_UNIT_NAME,
                                  LDAPUtils.POLICY_OU);
  ...
  @Override public Policy find(Account account,
                               String policyName) throws DataAccessException {
    Policy policy = new Policy();
    policy.setAccount(account);
    ...
    String ldapBase = String.format(
        ...
        LDAPUtils.ORGANIZATIONAL_UNIT_NAME, LDAPUtils.ACCOUNT_OU,
        LDAPUtils.BASE_DN);
    String filter = String.format("(%s=%s)", LDAPUtils.POLICY_NAME, policyName);
  ...
  @Override public void delete (Policy policy) throws DataAccessException {
    ...
    String dn = String.format(
        "%s=%s,%s=%s,%s=%s,%s=%s,%s", LDAPUtils.POLICY_ID, policy.getPolicyId(),
        LDAPUtils.ORGANIZATIONAL_UNIT_NAME, LDAPUtils.POLICY_OU,
        LDAPUtils.ORGANIZATIONAL_NAME, policy.getAccount().getName(),
        LDAPUtils.ORGANIZATIONAL_UNIT_NAME, LDAPUtils.ACCOUNT_OU,
        LDAPUtils.BASE_DN);
    ...
    try {
      ...
      LOGGER.error("Failed to delete the user: " + policy.getName());
