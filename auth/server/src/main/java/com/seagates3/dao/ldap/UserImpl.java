...
public class UserImpl implements UserDAO {
  ...
  private
    ...
    */
   @Override public User find(String accountName,
     ...
     if (ldapResults != null && ldapResults.hasMore()) {
       ...
       try {
         List<String> policyIds = new ArrayList<String>(Arrays.asList(
             entry.getAttribute(LDAPUtils.POLICY_ID).getStringValueArray()));
    ...
    */
  public
   User findByUserId(String accountName,
     ...
     try {
       ...
       LOGGER.error("Failed to find details of user: " + userId + " account: " +
                    accountName);
       throw new DataAccessException("Failed to find user details.\n" + e);
     }
     ...
     if (ldapResult != null && ldapResult.hasMore()) {
       ...
       if (user.getUserType() == User.UserType.IAM_USER) {
         user.setPath(ldapEntry.getAttribute(LDAPUtils.PATH).getStringValue());
   ...
   @Override public User[] findAll(String accountName, String pathPrefix)
     ...
     try {
       ...
       LOGGER.error("Failed to find all users of path prefix: " + pathPrefix +
                    " account: " + accountName);
    ...
    * @throws com.seagates3.exception.DataAccessException
    */
   @Override public void delete (User user) throws DataAccessException {
    ...
    * @throws com.seagates3.exception.DataAccessException
    */
   @Override public void update(User user, String newUserName,
                                String newPath) throws DataAccessException {
     ...
     LOGGER.debug("Updating user dn: " + dn + " new user name: " + newUserName +
                  " new path: " + newPath);
