...
public class UserPolicyResponseGenerator extends AbstractResponseGenerator {
	...
	public ServerResponse userPolicyAttachQuotaViolation() {
		String errorMessage = "Number of policies that can be attached exceed the limit of - "
				+ AuthServerConfig.USER_POLICY_ATTACH_QUOTA;
		return invalidRequest(errorMessage);
	}

	public ServerResponse userPolicyAlreadyAttached() {
		String errorMessage = "The request was rejected because it attempted "
				+ "to attach already attached policy to the user.";

		return formatResponse(HttpResponseStatus.CONFLICT, "EntityAlreadyExists", errorMessage);
	}

	public ServerResponse attachNonAttachablePolicy() {
		String errorMessage = "The request was rejected because it attempted "
				+ "to attach non attachable policy to the user.";

		return formatResponse(HttpResponseStatus.BAD_REQUEST, "InvalidParameterValue", errorMessage);
	}

	public ServerResponse detachNonAttachedPolicy() {
		String errorMessage = "The request was rejected because it attempted "
				+ "to detach a policy that is not attached to the user.";

		return formatResponse(HttpResponseStatus.NOT_FOUND, "NoSuchEntity", errorMessage);
	}
}
